class Commands::SetState < Command
  ...
  def do_action
    raise "state #{@state_name.inspect} doesn't exist for #{@character}" unless @character.animation_states.has_key? @state_name
    
    @prev_state_name = @character.animation_state_name
        
    unless @run_once
      @run_once = true
      
      prev_state = @character.instance_variable_get("@current_animation_state")
      
      if prev_state
        @unset_prev_state_events = prev_state.create_unset_events(@time)
      end
    
      @new_state = @character.animation_states[@state_name]
    ...
    @character.instance_variable_set("@current_animation_state", @new_state)
  ...
  def undo_action
    @character.instance_variable_set("@current_animation_state", @prev_state_name)
